...
func TestUnifiedResources_IdentityCenter(t *testing.T) {
	...
	setAccountAssignmentLabels := func(role types.Role) {
		r := role.(*types.RoleV6)
		r.Spec.Allow.AccountAssignmentLabels = types.Labels{
			types.OriginLabel: []string{apicommon.OriginAWSIdentityCenter},
		}
	}
					...
					AccountAssignment: &identitycenterv1.AccountAssignment{
						...
						Spec: &identitycenterv1.AccountAssignmentSpec{
							...
							PermissionSet: &identitycenterv1.PermissionSetInfo{
								Arn:            "some:arn",
								Name:           "Test Account",
								AssignmentName: "Test Assignment on Test Account",
							},
						},
					},
				})
		...
		t.Run(test.name, func(t *testing.T) {
			...
			t.Run("access via generic kind", func(t *testing.T) {
				user, _, err := CreateUserAndRole(srv.Auth(), "read-generic", nil,
					...
					WithRoleMutator(setAccountAssignmentLabels))
				require.NoError(t, err)
